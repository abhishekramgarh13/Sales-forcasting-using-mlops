...
@click.option(
    ...
    help="Optionally you can chose to only run the deployment"
    "pipeline to train and deploy a model (`deploy`), or to "
    "only run a prediction against the deployed model"
    "(`predict`). by default both will run be run "
    "(`deploy_and_predict`).",
)
@click.option(
    "--min-accuracy",
...
def main(config: str, min_accuracy: float):
    ...
    if deploy:
        continuous_deployment_pipeline(
            data_path="/home/abhishek13/Sales Forcasting/Sales-forcasting-using-mlops/data/Walmart Data Analysis and Forcasting.csv",
